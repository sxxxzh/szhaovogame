...
define(['player', 'entityfactory', 'lib/bison'], function(Player, EntityFactory, BISON) {
    ...
    var GameClient = Class.extend({
        ...
        connect: function(dispatcherMode) {
            ...
            // 如果 host 已包含协议（http/https/ws/wss），按其构造并忽略端口
            var hasScheme = host.indexOf('://') !== -1;
            var url;
            if (hasScheme) {
                // 将 http/https 映射到 ws/wss，并去除末尾多余斜杠
                url = host.replace(/^http:/, 'ws:').replace(/^https:/, 'wss:').replace(/\/+$/, '') + '/';
            } else {
                // 根据端口或页面协议选择 ws/wss；443 走 wss 并省略 :443
                var useSecure = (port === 443) || (typeof location !== 'undefined' && location.protocol === 'https:');
                var scheme = useSecure ? 'wss://' : 'ws://';
                var portPart = (port && !(useSecure && port === 443)) ? (":" + port) : '';
