    ...
    var Game = Class.extend({
        ...
        connect: function(started_callback) {
            ...
            this.client.onWelcome(function(id, name, x, y, hp) {
                ...
                self.client.onPlayerDamageMob(function(id, points) {
                    var entity = self.getEntityById(id);
                    if(entity && points) {
                        if(entity instanceof Player) {
                            // 非本地玩家的受击动画与伤害提示
                            if(id !== self.playerId) {
                                entity.hurt();
                                self.infoManager.addDamageInfo(-points, entity.x, entity.y - 15, "received");
                            }
                        } else {
                            // 保持对怪物的“造成伤害”显示逻辑
        ...
        onCharacterUpdate: function(character) {
            ...
            if(character.previousTarget && !character.isMoving() && character instanceof Mob) {
                ...
                // 目标可能已被移除（死亡/传送），避免持续访问不存在的实体导致报错
                if(this.entityIdExists(t.id)) {
                    character.previousTarget = null;
                    this.createAttackLink(character, t);
                    return;
                } else {
                    // 不存在则清理引用并停止尝试从旧目标恢复攻击
                    character.previousTarget = null;
                    character.disengage();
                    character.idle();
                }
