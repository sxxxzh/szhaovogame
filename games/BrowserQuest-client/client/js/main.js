...
define(['jquery', 'app'], function($, App) {
    var app, game;
    // SDK 昵称预取：优先使用登录昵称，失败则走原有输入/存档
    function prefillNicknameFromSDK() {
        try {
            // 允许通过 URL 参数覆盖 API 基址，例如 ?apiBase=https://gameapi.szhaovo.cn
            var params = new URLSearchParams(window.location.search);
            var apiBase = params.get('apiBase') || 'https://gameapi.szhaovo.cn';
            var token = localStorage.getItem('auth_token');
            if (!token) return; // 未登录则直接使用原逻辑
            ...
            .then(function(me) {
                var nickname = (me && (me.name || me.email || me.sub)) || '';
                if (nickname) {
                    // 预填并锁定输入框，避免被手动覆盖；保留显示以不破坏 UI
                    var $name = $('#nameinput');
                    $name.attr('value', nickname);
                    $name.prop('readonly', true);
                    // 同步到已玩档案区以保持一致体验
                    $('#playername').html(nickname);
                    // 确保播放按钮可用
                    app && app.toggleButton && app.toggleButton();
                }
            })
            .catch(function() {
                // 忽略错误，继续原有输入流程
